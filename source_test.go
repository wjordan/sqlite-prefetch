...
func (s *staticSource) Completeness() float64     { return s.complete }
func (s *staticSource) EgressCost() float64       { return s.egress }
...
func TestEstimatedTime_ZeroBandwidth(t *testing.T) {
	...
	got := estimatedTime(src, 4096)
	// With zero bandwidth, should return latency + MaxInt64/2 (effectively infinite).
	if got < time.Hour {
